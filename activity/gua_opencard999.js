...
3.关注5京豆
4.加购5京豆
  (默认不加购 如需加购请设置环境变量[guaopencard_addSku999]为"true"
5.抽奖 (默认不抽奖 如需抽奖请设置环境变量[guaopencard_draw999]为"3"
填写要抽奖的次数 不足已自身次数为准
guaopencard_draw999="3"
填非数字会全都抽奖
...
All变量适用
————————————————
入口：[ 自改999 (https://lzdz1-isv.isvjcloud.com/dingzhi/customized/common/activity?activityId=dzlhkkkbblnt20220121A&shareUuid=4a18d9d3a3a04d8abaff01e6e792f87d)]
...
30 3 19-28/3 1 * https://raw.githubusercontent.com/smiek2121/scripts/master/gua_opencard110.js, tag=1.21~1.28 自改999, enabled=true
...
guaopencard_addSku = $.isNode() ? (process.env.guaopencard_addSku999 ? process.env.guaopencard_addSku999 : `${guaopencard_addSku}`) : ($.getdata('guaopencard_addSku999') ? $.getdata('guaopencard_addSku999') : `${guaopencard_addSku}`);
...
guaopencard = $.isNode() ? (process.env.guaopencard999 ? process.env.guaopencard999 : `${guaopencard}`) : ($.getdata('guaopencard999') ? $.getdata('guaopencard999') : `${guaopencard}`);
...
guaopenwait = $.isNode() ? (process.env.guaopenwait999 ? process.env.guaopenwait999 : `${guaopenwait}`) : ($.getdata('guaopenwait999') ? $.getdata('guaopenwait999') : `${guaopenwait}`);
...
guaopencard_draw = $.isNode() ? (process.env.guaopencard_draw999 ? process.env.guaopencard_draw999 : guaopencard_draw) : ($.getdata('guaopencard_draw999') ? $.getdata('guaopencard_draw999') : guaopencard_draw);
...
!(async () => {
  if ($.isNode()) {
    if(guaopencard+"" != "true"){
      console.log('如需执行脚本请设置环境变量[guaopencard999]为"true"')
    }
    if(guaopencard+"" != "true"){
      return
    }
  ...
  $.shareUuid = "5c1e7599b34c4100bbc5b8968178591b"
  ...
  try {
    ...
    if(!$.addCart && !$.outFlag){
      if(guaopencard_addSku+"" == "true"){
        ...
        console.log('如需加购请设置环境变量[guaopencard_addSku999]为"true"');
      }
    }
    if(flag){
      await takePostRequest('activityContent');
    ...
    }else console.log('如需抽奖请设置环境变量[guaopencard_draw999]为"3" 3为次数');
